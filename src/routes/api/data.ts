  ...
  try {
    ...
    console.error(
      chalk.bold.redBright(
        `[ERROR ${dayjs().format("HH:mm:ss")}] Error fetching from TMDB! ${e}`,
      ),
    );
    res
      .status(500)
...
const fetchFromDG = async (
  ...
  try {
    ...
    console.error(
      chalk.bold.redBright(
        `[ERROR ${dayjs().format("HH:mm:ss")}] Error fetching from Zap2It! ${e}`,
      ),
    );
...
router.get("/image", async (req, res) => {
  ...
  try {
    ...
    console.error(
      chalk.bold.redBright(`Error fetching or processing image! ${error}`),
    );
    res
      .status(500)
  ...
  try {
    ...
    for (const provider of sortedProviders) {
      try {
        ...
        console.error(
          `[ERROR] Error fetching channel data for provider ID ${provider.id}: ${channelError}`,
        );
    ...
    res
      .status(500)
      .send({ error: "Failed to fetch data from the provider API." });
  }
});
...
router.get("/streamingProviders", async (req, res) => {
  //We will hardcode streaming providers since TheMovieDB.org pulls a lot of them + use the older images :)
